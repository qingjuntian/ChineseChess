...
public class ChessView extends View {
	...
	private final static PaintFlagsDrawFilter aliasFilter = new PaintFlagsDrawFilter(
	...
	private static Bitmap loadBmp(int id, int width, int height) {
		SvgDrawable svg = SvgDrawable.getDrawable(ChessApplication
				.getAppContext().getResources(), id);
	...
	private void drawBoard(Canvas canvas) {
		...
		for (int i = 0; i <= 8; i++) {
			if (i == 0 || i == 8) {
				canvas.drawLine(margin + cellSize * i, margin, margin
						+ cellSize * i, margin + cellSize * 9, linePaint);
			} else {
				canvas.drawLine(margin + cellSize * i, margin, margin
						+ cellSize * i, margin + cellSize * 4, linePaint);
		...
		// Draw h-lines
		for (int j = 0; j <= 9; j++) {
			canvas.drawLine(margin, margin + cellSize * j, margin + cellSize
					* 8, margin + cellSize * j, linePaint);
		}
		// Draw cross-lines
		canvas.drawLine(margin + cellSize * 3, margin, margin + cellSize * 5,
				margin + cellSize * 2, linePaint);
		canvas.drawLine(margin + cellSize * 5, margin, margin + cellSize * 3,
				margin + cellSize * 2, linePaint);
		canvas.drawLine(margin + cellSize * 3, margin + cellSize * 7, margin
				+ cellSize * 5, margin + cellSize * 9, linePaint);
		canvas.drawLine(margin + cellSize * 5, margin + cellSize * 7, margin
				+ cellSize * 3, margin + cellSize * 9, linePaint);

		linePaint.setStrokeWidth(3);
		// Draw outer line
		canvas.drawRect(margin - 3, margin - 3, margin + cellSize * 8 + 3,
	...
	private void drawChessmen(Canvas canvas, int x, int y, byte chessman) {
		int cx = margin + x * cellSize;
		int cy = margin + y * cellSize;
		if (chessman < 16) {
			canvas.drawBitmap(redChess, cx - (int) (cellSize * 0.97) / 2, cy
					- (int) (cellSize * 0.97) / 2, null);
		} else {
			canvas.drawBitmap(blackChess, cx - (int) (cellSize * 0.97) / 2, cy
					- (int) (cellSize * 0.97) / 2, null);
		}

		Bitmap txtBmp = getBitmap(chessman);
		canvas.drawBitmap(txtBmp, cx - (int) (cellSize * 0.8) / 2, cy
				- (int) (cellSize * 0.8) / 2, null);
	}

	private void drawSelect(Canvas canvas) {
		if (selected
				&& engine.getChessmanColor(selectX, selectY) == engine
						.getDirection()) {
			int cx = margin + selectX * cellSize;
			int cy = margin + selectY * cellSize;
			canvas.drawRoundRect(new RectF(cx - cellSize / 2 - 2, cy - cellSize
					/ 2 - 2, cx + cellSize / 2 + 2, cy + cellSize / 2 + 2), 4,
					4, selectPaint);
		}
	}
	...
	protected void onDraw(final Canvas canvas) {
		...
		if (!isInEditMode()) {
			if (svgs == null) {
				svgs = new Bitmap[] {
						loadBmp(R.raw.che, txtSize, txtSize),
						loadBmp(R.raw.ma, txtSize, txtSize),
						loadBmp(R.raw.pao, txtSize, txtSize),
						loadBmp(R.raw.bing, txtSize, txtSize),
						loadBmp(R.raw.zu, txtSize, txtSize),
						loadBmp(R.raw.shuai, txtSize, txtSize),
						loadBmp(R.raw.jiang, txtSize, txtSize),
						loadBmp(R.raw.r_shi, txtSize, txtSize),
						loadBmp(R.raw.b_shi, txtSize, txtSize),
						loadBmp(R.raw.r_xiang, txtSize, txtSize),
						loadBmp(R.raw.b_xiang, txtSize, txtSize) };
			}
			if (blackChess == null)
				blackChess = loadBmp(R.raw.black, outSize, outSize);
			if (redChess == null)
				redChess = loadBmp(R.raw.red, outSize, outSize);
		}
		// Draw chess board.
		drawBoard(canvas);
